...
Daily rainfall chart for **KEMP — Emporia Municipal Airport, Kansas**, updated automatically every 6 hours and published to GitHub Pages.
...
| `.github/workflows/update.yml` | The 6-hour cron job that runs `run_update.py` on GitHub's runners and commits results back. |
...
That fires at `00:00, 06:00, 12:00, 18:00` UTC. GitHub Actions cron can be delayed 5–15 minutes during peak load, but a missed run is harmless — the next run still picks up everything new (the NWS obhistory page retains ~3 days, so we have a wide overlap).
...
- **3-day data window from NWS.** The obhistory page only retains roughly the last 3 days. We rely on the every-6-hour scrape to keep building up history in `KEMP_obhistory_running.csv`. As long as we don't miss runs for more than ~3 days in a row, all observations get captured. If a longer gap forms, those observations are gone — there's no way to backfill them from this source. For deeper historical data, use the [Iowa State ASOS archive](https://mesonet.agron.iastate.edu/request/download.phtml) instead.
